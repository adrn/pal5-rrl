...
\shorttitle{RRL in the Pal 5 stream}
...
\title{Kinematics of the Palomar 5 stellar stream from RRLs}
...
% \sa{The above sentence makes it sound like $\lesssim 20\%$ of the 150 known GCs have streams, is it really that many?}
% APW: Yep! According to the citations.
...
% \sa{Maybe cite Dotter 2011 for previous distance? }
% APW: It's just a qualitative statement to give a rough sense of its distance, so I'm not really quoting a measurement
...
% \sa{Might be work stating how big the estimated mass difference is in the trailing, leading vs cluster, as many people seem to be curious about this.}
% APW: Added a note above
Given that Pal 5's pericenter is well within the Galactic disk ($r_{\textrm{peri}} \sim 7$--$8~\kpc$),
% \sa{cite Erkal 2017 here (his fig 6)?}
% APW: Again just a rough / qualtitative number to give a sense - I don't really see the need to cite given that this is from my own exploration with the orbit fits!
...
% \sa{Maybe here mention that a "velocity" gradient has been detected along the stream (Kuzma 2015)? }
% APW: Good idea, but then it would make sense to talk about the RV data in Ibata+2017. I'll try to fit both into the discussion.
...
% \sa{Might be worth looking at closer distances (leading arm in a MW potential comes as close as $\sim$15 kpc), especially if you want to find members at a dec of $-15\deg$.}
% APW: Hm, my models only go to ~18 at the edge of the sky footprint, but this distance cut is only used for illustrative purposes. I added a sentence to clarify.
...
% \sa{Do we want to show this stream track? Or state how wide it is? (it's inthe  figure, but mention here too).}
% APW: Noted.
...
% \sa{I got a little confused in the section above as Fig 3 is referenced but Fig 2 never mentioned. }
% APW: I moved this paragraph up from the discussion and forgot to edit - thanks!
...
% \sa{Is it worth it to show the whole region in fig 3, and/or maybe extend to even more negative $\delta $?}
% APW: I don't think so? And what do you mean by the "whole region"?
%SP meant whether you found any RRLs below $\delta<-10$ (now found answer below so never mind)
...
% \sa{minor: three sentences in a row start with "we use".}
% APW: changed one of them
...
% \sa{Could be worth emphasizing here too that the probability does not depend on positional info (i.e. being close to the stream itself) for people only skimming through figures.}
% APW: added!
...
\caption{Sky positions of RRLs with Pal 5 membership probabilities $>0.01$ \todo{CM: APW check this number, I think it might be 0.1?}, colored by membership probability \sout{(large markers)}. The background greyscale shows the surface density of CMD-filtered main sequence stars from \citet{Bonaca:2019}. Note that the membership probability does not incorporate sky position information, but a clear over-density of high-probability RRL members do trace the stream on the sky.}
\label{fig:members}
\end{center}
\end{figure*}
...
% \sa{What about RRLs at more negative $\delta$, wasn't your region larger than the figure extent?}
% APW: There aren't any stars with prob > 0.01 below dec ~ -11 near the stream, so I trimmed the axis.
...
We find a proper motion $(\mu_\alpha\,\cos\delta, \mu_\delta) = (-2.69, -2.70) \pm (0.04, 0.03)~\masyr$, consistent with the measurement in \citet{Vasiliev:2019}. % \sa{state that this is larger than most before next sentence?}
...
% \sa{you mean only the cluster stars, right?}
% APW: no, I mean all! Because of our quadratic model, even stars in the nearby part of the stream technically inform the cluster position
%SP: cool, does the distance estimate change at all if you only use cluster stars?
% APW: they are consistent
...
% \sa{which has been suggested in/reported in/consistent with modeling of the stream...? fig 2, pearson 17}
% APW: Not really worth saying consistent or not because of the distance difference, but I added the citation!
