...
% Methods
% Results
By building a probabilistic model of the Pal 5 \changes{cluster and stream} in proper motion and distance, we find \NRRL\ RRLs consistent with being members of the cluster (\NRRcl) and stream (\NRRtails).
...
\figurename~\ref{fig:pm} demonstrates that Pal 5 cluster and stream members appear in the RRL catalog: This figure shows the \Gaia\ \DR{2} proper motions for all RRLs in the selected sky window (indicated in the caption) within the distance range $18 < d < 25~\kpc$, then the same for RRLs in a tight selection around the observed stream track (see \sectionname~\ref{sec:membership} for details), and then finally the same for RRLs \emph{excluding} the stream region.
