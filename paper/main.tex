...
\ab{mention RRL overdensities along the tails don't correspond to MS overdensities?}

\subsection{Kinematics of the cluster and stream}

\ab{this section is mostly about the distances -- include in the subtitle?}
...
\ab{can we work this point a bit more? I thought it was mainly the HST CMD measurement from Aaron that was an outlier, and everything else at a larger distance. then I think in Nitya + Tobias paper they found that to match pms, they require a smaller distance, and now we find it in PS1 RRL. I don't think we need to recount the whole history in a letter, but it might be helpful to comment on the difference wrt Kathy's RRL result}
...
\ab{can we provide a sentence for what it (physically) means to be an oosterhoff I population?}
...
\ab{this is interesting indeed -- can we provide a hypothesis?}
