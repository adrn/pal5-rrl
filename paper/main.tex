...
\sa{throughout the manuscript, check if you mean plural when using RRL vs RRLs.}
...
\sa{if you say the "shape and mass", you can cite Pearson 15 too ;)}
...
However, at its present distance, main sequence stars are barely detected and have large astrometric uncertainties in \Gaia\ \DR{2}, 
\sa{The above makes it sound like the ms stars are barely detected in general, maybe flip sentence so it's clear that *in Gaia*....}
and other stellar tracers (e.g., giant branch stars) are not numerous enough or precise enough distance indicators to provide useful relative distance information along the stream.
...
% APW: My models only go to 18 at the edge of the sky footprint, but this distance cut is only used for illustrative purposes. I added a sentence to clarify.
...
\caption{RRL stars in the vicinity of the Pal 5 stream (with $\textrm{prob.} > 0.01$), colored by membership probability, and under-plotted with the inferred stream trends used in the membership model. \sa{Could be worth emphasizing here too that the probability does not depend on positional info (i.e. being close to the stream itself) for people only skimming through figures.}
...
\caption{Sky positions of RR Lyra stars with Pal 5 membership probabilities $>0.01$, colored by membership probability (large \sa{circular?} markers). The background greyscale shows the surface density of CMD-filtered main sequence stars from \citet{Bonaca:2019}. \sa{specify that RA/dec are in degrees?}}
\label{fig:members}
\end{center}
\end{figure*}
...
\figurename~\ref{fig:trackmembers} shows a summary of the inferred membership model properties in sky position (shown in Pal 5 coordinates $\phi_1, \phi_2$) \sa{might be worth emphasizing again that this is along the stream of pal 5 and references Ana's figure (people aren't used to seeing the stream in this coord system.}, distance, and proper motion components.
...
No theoretical models of Pal 5 have predicted tidal debris at such locations, so follow-up spectroscopy to confirm or rule out their origin would be very informative. \sa{What about RRLs at more negative $\delta$, wasn't your region larger than the figure extent?}


\subsection{Kinematics of the cluster and stream}

Past distance measurements to the Pal 5 cluster have ranged from $20.9~\kpc$ \citep{Dotter:2011} \sa{worth mentioning with which technique?} to $23.2~\kpc$ \citep{Harris:1996}, with a previous measurement from a small number of RR Lyrae stars in between at $22.3~\kpc$ \citep{Vivas:2006}.
...
The larger proper motion (relative to \citealt{Fritz:2015} and other earlier measurements) and the closer distance means that the tangential velocity of Pal 5 is $\approx 30~\kms$ larger than most previous models have assumed. \sa{which will be crucial/important for future modeling, migth change inferred inclosed mass of kuepper 2015 etc etc. More a discussion point I guess, will read.}
...
While the proper motion in right ascension appears consistent with constant or only a mild gradient over the extent of the stream, we see a significant gradient in the declination component, with a difference of about $\approx 1~\masyr$ from trailing to leading tail. \sa{worth mentioning Pearson 2017, fig 2 where this same trend is also seen (flatter gradient in RA proper motion than in dec proper motion?}
...
Assuming a Jacobi radius of $12^\prime$ (computed assuming $\textrm{M}_{\rm Pal 5} \approx 1.4\times10^4~\msun$ \sa{ref for mass?} and using the Milky Way mass model in \citealt{gala}), we find 15 RRLs in the cluster tails, 11 classified as type~\typeab\ and 4 type~\typec.
...
The full sample of PS1 RRLs is shown in the background for comparison, as a grayscale density histogram. The distribution of the \typeab~RRL clearly indicate that Pal~5 is an Oosterhoff I cluster, consistent with having a mean period of $0.565~d$ \todo{(XX) ?}.
...
This type of RRL is only found in relatively metal-rich globular clusters  \citep[$\FeH>-1.5$;][]{Monelli:2017} and the Sagittarius dwarf galaxy, but not in any other dwarf spheroidal galaxy. \todo{CM:( any other point to be made about this?)}. \sa{as someone who doesn't know much about RRLs, how weird is the above? maybe some more discussion/elaboration? }


\begin{figure*}[t]
\begin{center}
\includegraphics[width=\textwidth]{rrls_PA.pdf}
\caption{Amplitude ($g$-band) versus Period for RRLs in the Pal~5 field. \sa{some statement on what to take away from the figure... i.e. consistent with Pal 5 being an "x" type GC....}}
...
The Pal~5 cluster has 5 previous known RRL stars (named V1--V5), all of type which are \typec, as reported in the \citet{Clement:2001} compilation of variable stars in Galactic globular clusters and dating back to \citet{SawyerHogg:1973}.
...
From these stars \sa{you mean only the cluster stars, right?}, we infer the distance to the cluster, finding a closer value, $d = 20.9 \pm 0.2~\kpc$, than many past studies, and detect a distance gradient of $\sim 0.4~\kpc~{\rm deg}^{-1}$ between the trailing and leading tail (between $-5^\circ < \phi_1 < 5^\circ$).
...
These data will be instrumental for improving dynamical models of Pal 5, both for using Pal 5 to constrain the global matter distribution of the Milky Way and in interpreting the observed, complex density structure of the stream. \sa{not sure where in text, but could be worth mentioning that you don't find stars beyond the current footprint of the stream (i.e. that leading stream does appear to be shorter than trailing..., although looking at fig 3, you actually don't find trailin stream RRLs much further from the prog. than you find leading arm RRLs.}
