...
The Palomar 5 stellar stream (Pal 5) is an exemplar stream within the Milky Way: Its $\approx 20\degr$ stream connects back to the progenitor cluster, and it has been used to study the shape and total mass of, along with substructure in, the dark matter distribution of the Galaxy.
However, ...most details of the stream are not fully explained.
% Aims
...we want to measure distance and kinematic tracks along the stream to inform models.
Here we use a large catalog of RR Lyrae-type stars (RRLs) with astrometric data from the \Gaia\ mission to probabilistically identify RRLs in the Pal 5 stream.
