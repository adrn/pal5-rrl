...
%     - APW: I tried running with these as parameters, but they are unconstrained, so doesn't help. I'll just set the dispersions to small (<uncertainties) values and explain.
...
\figurename~\ref{fig:pm} demonstrates that Pal 5 stream members appear in the RRL catalog: This figure shows the \Gaia\ \DR{2} proper motions for all RRLs in the selected sky window within the distance range $18 < d < 25~\kpc$, then the same for RRLs in a tight selection around the observed stream track (within $1\degr$ of the stream track, see \sectionname~\ref{sec:membership} for details), and then finally the same for RRLs \emph{excluding} the stream region (excluding stars within $2\degr$ of the stream track).
